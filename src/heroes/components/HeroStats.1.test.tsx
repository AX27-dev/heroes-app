...
import {
  ...
  type FavoriteHeroContextType,
} from '../context/FavoriteHeroContext';
...
const renderHeroStats = (
  ...
  if (favoriteCount !== undefined) {
    const mockFavoriteHeroContext = {
      favoriteCount,
    } as FavoriteHeroContextType;

    return render(
      <QueryClientProvider client={queryClient}>
        <FavoriteHeroContext value={mockFavoriteHeroContext}>
          <HeroStats />
        </FavoriteHeroContext>
      </QueryClientProvider>
    );
...
describe('HeroStats', () => {
  ...
  test('should change the percentage of favorites when a hero is added to favorites', () => {
    renderHeroStats(mockSummaryData, 22);

    const favoritePercentageElement = screen.getByTestId('favorite-percentage');
    expect(favoritePercentageElement.innerHTML).toBe('88.00% of total');

    const favoriteCountElement = screen.getByTestId('favorite-count');
    expect(favoriteCountElement.innerHTML).toBe('22');
